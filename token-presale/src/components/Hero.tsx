...
import { calculateTokens, KTK_TOKEN_ADDRESS } from '@/utils/contract'
...
export default function Hero() {
  ...
  const progress = presaleData?.progress ?? 0
  ...
  const addTokenToWallet = async () => {
    if (!isConnected) {
      showNotification({
        ...
        message: 'Please connect your wallet first to add KTK token.',
        duration: 4000
      })
      return
    }
    ...
    try {
      ...
      if (typeof window.ethereum !== 'undefined') {
        ...
        if (wasAdded) {
          showNotification({
            type: 'success',
            title: 'Token Added!',
            message: 'KTK token has been added to your wallet successfully.',
            duration: 5000
          })
        }
      } else {
        showNotification({
          ...
          message: 'Please install MetaMask or use a compatible wallet.',
          duration: 5000
        })
      }
    } catch (error) {
      ...
      showNotification({
        ...
        message: 'Could not add KTK token to your wallet. Please try again.',
        duration: 5000
      })
    }
  }
        ...
        <div className="flex items-center" style={{paddingLeft: '7.5%', paddingRight: '7.5%', gap: '10%'}}>
              ...
              <div className="relative bg-gradient-to-br from-black/60 to-gray-900/60 backdrop-blur-xl border border-orange-500/30 rounded-3xl" style={{padding: '25px', width: '100%'}}>
                ...
                <div>
                  ...
                  <div className="flex justify-between text-sm text-gray-400">
                    ...
                    <span>Goal: {presaleData ? `${parseFloat(presaleData.maxGoal)} ETH` : 'Loading...'}</span>
                  </div>
                </div>
                <br />
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">{presaleData ? `${presaleData.tokenPrice} ETH` : 'Loading...'}</span>
                  </div>
                  <div className="flex justify-between">
          ...
          <div className="order-1 lg:order-2 text-center lg:text-left" style={{width: '50%'}}>
            ...
            >
              ...
              <motion.button
                whileHover={{ scale: isConnected ? 1.1 : 1.0 }}
                whileTap={{ scale: isConnected ? 0.95 : 1.0 }}
                onClick={addTokenToWallet}
                disabled={!isConnected}
                className={`group border-2 rounded-2xl font-bold text-lg transition-all duration-300 backdrop-blur-sm flex items-center ${
                  isConnected 
                    ? 'border-orange-500/30 hover:border-orange-500 bg-black/20 hover:bg-orange-500/10 text-white cursor-pointer' 
                    : 'border-gray-600/30 bg-gray-800/20 text-gray-500 cursor-not-allowed'
                }`}
              >
                <Plus className={`mr-2 w-5 h-5 ${isConnected ? 'text-orange-500' : 'text-gray-500'}`}/>
                <span className="relative z-10 inline-block" style={{padding: '8px 20px'}}>
                  Add KTK to wallet
                </span>
              </motion.button>
            </motion.div>
